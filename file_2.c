...
string binary_file_path = "output/list_binary.txt";

void saveOnFile(void);
void saveOnBinaryFile(void);
...
int menu(){
    ...
    printf("5. Exportar arquivo (binário). \n");
    printf("6. Ler do arquivo (binário). \n");
...
void saveOnBinaryFile(void){
    FILE *file;
    file = fopen(binary_file_path, "wb"); // retorna NULL se nao for possivel criar ou abrir o arquivo
    if(!file) {
        printf("erro ao abrir arquivo. \n");
        return;
    }; 
    fwrite(&quantity_list, sizeof(int), 1, file);
    fwrite(list, sizeof(person_t), quantity_list, file);
    // array de struct, tamanho de cada struct, quantidade de elementos, arquivo

    printf("Arquivo exportado com sucesso (binário). \n");
    fclose(file);
};
void readBinaryFile(void){
    FILE *file;
    file = fopen(binary_file_path, "rb"); // retorna NULL se nao for possivel criar ou abrir o arquivo
    if(!file) {
        printf("erro ao abrir arquivo. \n");
        return;
    }; 
    ...
    fread(&recovered_quantity_list, sizeof(int), 1, file);
    fread(list, sizeof(person_t), recovered_quantity_list, file);

    printf("Número de cadastros: %i. \n", recovered_quantity_list);

    quantity_list = recovered_quantity_list;
    printf("Arquivo lido com sucesso (binário). \n");
    fclose(file);
};
void printArray(void){
    int counter = 0; 
